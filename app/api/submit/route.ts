...
import { publishProof, publishCertificateLink } from "@/lib/hedera/hcs";
import { mintProofToken } from "@/lib/hedera/hts";
import { scheduleReward } from "@/lib/hedera/schedule";
import { uploadToHFS } from "@/lib/hedera/hfs";
import { generateCertificateHTML } from "@/lib/hedera/certificate";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── 4. Generate & upload HTML certificate to HFS ──────────────────────
    let hfs: {
      fileId: string; size: number; transactionId: string;
      certificateUrl: string; explorerUrl: string;
    } | null = null;
    ...
    try {
      const certHTML = generateCertificateHTML({
        ...
        hcsTxId:        hcs.transactionId,
        htsTxId:        hts.transactionId,
        isPrivate,
        network: process.env.HEDERA_NETWORK ?? "testnet",
      });

      hfs = await uploadToHFS(
        certHTML,
        `PC:${docHash.slice(0, 12)} ${filename.slice(0, 40)}`,
      );

      console.log(`📁 HFS certificate: ${hfs.fileId} (${hfs.size} bytes)`);

      // ── 5. Publish CERTIFICATE_LINK on HCS ───────────────────────────────
      // A tiny second HCS message that links docHash → hfsFileId.
      // The dashboard reads these to resolve "Open Certificate" for past entries.
      try {
        await publishCertificateLink({
          docHash,
          hfsFileId: hfs.fileId,
          hcsSeq:    hcs.topicSequenceNumber,
        });
        console.log(`🔗 CERTIFICATE_LINK published for seq #${hcs.topicSequenceNumber}`);
      } catch (e: any) {
        console.warn("CERTIFICATE_LINK publish skipped:", e.message);
      }
    } catch (e: any) {
      console.warn("HFS upload skipped:", e.message);
    }
    ...
    return NextResponse.json({
      ...
      hfs: hfs ? {
        fileId:         hfs.fileId,
        size:           hfs.size,
        transactionId:  hfs.transactionId,
        certificateUrl: hfs.certificateUrl,
        explorerUrl:    hfs.explorerUrl,
      } : null,

      schedule: schedule
        ? { scheduleId: schedule.scheduleId, transactionId: schedule.transactionId }
        : null,
      ...
      explorerLinks: {
        ...
        ...(hfs ? { certificate: hfs.certificateUrl } : {}),
      },
    });
  } catch (err: any) {
