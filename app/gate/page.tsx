...
const ENV    = (process.env.NEXT_PUBLIC_WORLD_ENV  ?? "staging") as "staging" | "production";
const IS_DEV = !process.env.NEXT_PUBLIC_WORLD_APP_ID && typeof window !== "undefined";

type Step = "idle" | "loading" | "qr" | "polling" | "verifying" | "done" | "error";
...
function GateContent() {
  ...
  const verify = useCallback(async () => {
    ...
    try {
      ...
      if (uri) {
        setQrUrl(
          `https://api.qrserver.com/v1/create-qr-code/?size=240x240&margin=10&color=ffffff&bgcolor=000000&data=${encodeURIComponent(uri)}`,
        );
        setLink(uri);
      }
      setStep("qr");
      ...
      <div style={s.card}>
        ...
        <div style={s.orbWrap}>
          <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
            <circle cx="32" cy="32" r="31" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
            <circle cx="32" cy="32" r="22" stroke="rgba(255,255,255,0.12)" strokeWidth="1" />
            <circle cx="32" cy="32" r="13" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5" fill="rgba(255,255,255,0.04)" />
        ...
        <p style={s.sub}>
          A notary is, by definition, a human being. Before you can certify
          that a document existed, prove that <em>you</em> do.
        </p>
        ...
        <div style={s.features}>
          {[
            { icon: "◎", title: "One person, one notary",  desc: "Each human gets access once. Bots and duplicates cannot." },
            { icon: "⊘", title: "Zero data shared",        desc: "We receive a ZK proof — not your identity or biometrics." },
            { icon: "↺", title: "Reusable credential",     desc: "Verify once with World ID. Works across apps." },
          ].map((f) => (
        ...
        <div style={s.action}>
          ...
          {(step === "qr" || step === "polling") && qrUrl && (
            <div style={s.qrBox}>
              <div style={s.qrLabel}>
                {step === "polling" ? "Waiting for World App…" : "Scan with World App"}
              </div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={qrUrl} alt="World ID QR code" width={200} height={200}
                style={{ display: "block", margin: "0 auto", borderRadius: 8 }} />
              {link && (
                <a href={link} style={s.deepLink} target="_blank" rel="noreferrer">
                  Open in World App ↗
                </a>
              )}
              {step === "polling" && (
                <div style={s.pollRow}>
                  <Spinner /> <span style={{ color: "#555", fontSize: 12 }}>Waiting for confirmation…</span>
                </div>
              )}
            </div>
          )}
          ...
          {/* States */}
          {step === "loading"   && <StatusRow><Spinner /><span>Preparing verification…</span></StatusRow>}
          {step === "verifying" && <StatusRow><Spinner /><span>Verifying proof…</span></StatusRow>}
          {step === "done"      && <StatusRow color="#4ade80"><CheckIcon /><span>Verified — entering ProvenanceChain…</span></StatusRow>}
        ...
        <p style={s.foot}>
          Powered by{" "}
          <a href="https://world.org" target="_blank" rel="noreferrer" style={s.footLink}>World ID</a>
          {" "}· Zero-knowledge proof of humanity
          {ENV === "staging" && (
            <> · <a href="https://simulator.worldcoin.org" target="_blank" rel="noreferrer" style={s.footLink}>Use simulator ↗</a></>
          )}
        </p>
      </div>
    </div>
...
export default function GatePage() {
  return <Suspense><GateContent /></Suspense>;
}
...
function CheckIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" style={{ flexShrink: 0 }}>
      ...
      <path d="M5.5 9l2.5 2.5 4.5-4.5" stroke="#4ade80" strokeWidth="1.4" strokeLinecap="round" />
    </svg>
  );
}
...
function StatusRow({ children, color }: { children: React.ReactNode; color?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, color: color ?? "#888",
...
const s: Record<string, React.CSSProperties> = {
  ...
  h1: {
    fontSize: "clamp(22px,4vw,30px)", fontWeight: 700, color: "#fff",
    textAlign: "center", lineHeight: 1.2, marginBottom: 12, letterSpacing: "-0.4px",
  },
  h1dim:    { color: "rgba(255,255,255,0.3)" },
  sub: {
    fontSize: 13.5, color: "rgba(255,255,255,0.4)", textAlign: "center",
    lineHeight: 1.75, marginBottom: 0,
  },
  rule:     { height: 1, background: "rgba(255,255,255,0.07)", margin: "24px 0" },
  features: { display: "flex", flexDirection: "column", gap: 14, marginBottom: 28 },
  feat:     { display: "flex", alignItems: "flex-start", gap: 13 },
  featIcon: { fontSize: 16, color: "rgba(255,255,255,0.4)", width: 20, textAlign: "center", flexShrink: 0, marginTop: 1 },
  featTitle:{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.75)", marginBottom: 2 },
  featDesc: { fontSize: 12, color: "rgba(255,255,255,0.3)", lineHeight: 1.5 },
  action:   { display: "flex", flexDirection: "column", gap: 10, alignItems: "center" },
  btn: {
    display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 9,
    width: "100%", padding: "13px 20px", background: "#fff", color: "#0a0a0b",
    border: "none", borderRadius: 10, fontSize: 15, fontWeight: 600,
    cursor: "pointer", letterSpacing: "-0.2px", transition: "opacity 0.15s",
  },
  qrBox: {
    width: "100%", background: "#000", border: "1px solid rgba(255,255,255,0.07)",
    borderRadius: 12, padding: "20px 16px", textAlign: "center",
  },
  qrLabel: {
    fontSize: 11, color: "rgba(255,255,255,0.3)", marginBottom: 14,
    letterSpacing: "0.8px", textTransform: "uppercase",
  },
  deepLink: { display: "block", marginTop: 12, fontSize: 12, color: "rgba(255,255,255,0.25)", textDecoration: "none" },
  pollRow:  { display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 14 },
  errBox: {
    width: "100%", padding: "12px 16px", textAlign: "center",
    background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)",
    borderRadius: 8, color: "#f87171", fontSize: 13, lineHeight: 1.5,
  },
  foot: { marginTop: 22, fontSize: 11, color: "rgba(255,255,255,0.18)", textAlign: "center", lineHeight: 1.6 },
